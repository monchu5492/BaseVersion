  ...
  async.waterfall(
    [
      ...
      function(token, done) {
        ...
        User.findOne({ where: { email: req.params.email } }).then(
          (founduser) => {
            if (!founduser) {
              console.log(
                `No user found with that email address ${req.params.email}`
              );
    ...
    [
      function(done) {
        User.findOne({
          ...
          if (!founduser) {
            const resetError =
              "30400: Password reset token is invalid or has expired.";
            return res.redirect(`/message/${resetError}`);
          }
          console.log(req.body.strength);
          if (req.body.strength !== "green") {
            const strengthError =
              "Password length, characters are not sufficient";
          ...
          const userSuccessMessage =
            "20000: Success! Account password updated.";
          return res.redirect(`/message/${userSuccessMessage}`);
        });
      },
    ],
