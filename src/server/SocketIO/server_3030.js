...
const getGameExpired = function (callback) {
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.query('SELECT expired_status FROM tournament WHERE' +
      ' tournament_id=(SELECT tournament_id FROM game' +
      " WHERE game_id='6da7df34-132f-45a7-9b8e-df253eca749e')", (err, rows) => {
      connection.release();
...
const getScores = function (callback) {
  pool.getConnection((err, connection) => {
    ...
    connection.query('SELECT SUM(total_score) AS ScoreTotal FROM finalscore ' +
      "WHERE game_id ='6da7df34-132f-45a7-9b8e-df253eca749e'", (err, rows) => {
      connection.release();
      // console.log(`SELECT * FROM fbstatus ORDER BY status_id DESC LIMIT 1 resulted in =>`);
      console.log(rows);
